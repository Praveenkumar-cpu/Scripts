...
plid_list = [
"c60ef089-ecf9-4b9d-82ee-20dedba8bb93",
"LAI1009748748",
"CASKB250316EIJQA"
]
failed_plid = []
...
def get_pending_task(plid):
    ...
    if not pending_response['taskList']:
        failed_plid.append(plid)
        return
    pending_task = pending_response['taskList'][0]['taskName']
    if last_task == pending_task:
...
def run_script(plid_list):
    for plid in plid_list:
        try:
            ...
            time.sleep(0.1)  # Sleep for 0.1 seconds to avoid bombarding the server
